...
Security fixes are currently provided for the latest commit on the default branch. M0–M9 implementation is complete. M6 provides file-backed authentication, trusted HTTPS/TURN deployment, strict Origin/Host authorization, bounded failed-authentication rate limiting, source recovery, structured audit, bounded logs, and detail-free probes. M8/M9 route the loopback-only NVR, playback and evidence APIs through the same control boundary, restrict media operations to stable catalog IDs, and redact all source endpoints and physical paths. Authentication remains disabled in base loopback Compose. Do not expose that base backend directly to a LAN or the Internet; use the documented production overlay and least-privilege network policy.

安全修复仅面向默认分支的最新提交；M0–M9 实现已完成。M6 已提供文件型认证、受信 HTTPS/TURN 部署、严格 Origin/Host 授权、有限认证失败限流、来源恢复、结构化审计、有界日志及无细节探针。M8/M9 的仅回环 NVR、回放和证据接口通过同一控制边界提供，媒体操作只接受稳定目录 ID，全部来源端点和物理路径均脱敏。基础回环 Compose 仍默认不启用认证；不得将其后端直接暴露到局域网或互联网，应使用已记录的 production 覆盖和最小权限网络策略。
