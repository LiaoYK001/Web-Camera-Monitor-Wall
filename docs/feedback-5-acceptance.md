...
## F5-05 多音轨 / per-source audio tracks — 逐轨探测/通道/前端混音/Scene v6 持久化与引擎逐轨混音均已实现
...
- **引擎逐轨混音（本轮新增）**：`resolved_audio_inputs()`（放在可单测的库单元里）决定引擎要混的输入轨：显式 `audioInputs` 优先，否则回退 legacy `audio_track`；每条输入轨生成一个独立 OBS 源实例（`ffmpeg_source` 的 `track` 属性 = 输入轨序号），各自应用 gain/mute 后送入**同一个节目总线（mixer 1）**。顺带修正了一个真实缺陷：旧实现把输入轨序号映射成 **输出调音台位**（`1U << (audio_track-1)`），因此 `audioTrack > 1` 的来源在节目里其实是**没有声音**的；现在输入轨只由 `track` 选择，输出统一走节目总线。复用路径（连接未变）会按新配置重建附属实例；输入轨变化则重建源，实例析构时成对 `obs_source_dec_active`。
  - 单元测试：`resolved_audio_inputs` 的显式优先与 legacy 回退；既有 `webobs-unit-tests` 全绿。
  - 运行期实测（合成 2 音轨来源：a:0=440Hz、a:1=880Hz；场景 `audioInputs=[{track:0},{track:1}]`）：引擎日志出现独立实例 `WebOBS rtsp synth.audio-1 audio track 2` 及其连接/重连记录，MediaMTX 该路径 **readers=2**（主源 + 附属实例各一条连接）；抓取节目音频做 Goertzel 分析：**附属实例的 880Hz 稳定出现在节目里**（多次抓取 e880≈1.6–1.7e7），单输入场景（`[{track:1}]` → 主源 `track=2`）也得到 880Hz，证明主源按 `track` 解码所选输入轨。
  - **尚未证明**：一次抓取里两路音轨**同时且稳定**出现。合成来源下引擎的来源健康看门狗反复重启主源（`source_health unavailable → recovered`、`restart_count` 递增），主源的 440Hz 在 4 次抓取中缓慢爬升（−61 → −47 → −31 dB，而附属实例的 880Hz 始终稳定），因此“两轨同时出流”的稳定证据仍需在来源健康的场景中复测；这也说明需要在下一轮确认健康恢复是否对合成/低帧率来源过于激进。

未实现 / not implemented（明确列为后续项）：
- 逐轨同步偏移仍按来源统一处理；附属实例目前不参与健康看门狗（只看主源）。
