...
- **服务端长稳（部分实测）**：真实五路来源降负载场景（960×540@5、软件渲染）连续运行 14 分钟、15 次每分钟采样：program 路径每次都 ready=True 且 tracks=['Opus','H264']，bytesReceived 由 182 KB 稳定增长到 57.9 MB（约 4 MB/分钟），webobsd 始终存活，source_recovery 与崩溃计数始终为 0（crashes=0 restarts=0），启动器报告 进程存活=是／引擎就绪=是／Program 发布=是／轨道=Opus,H264。该证据限于**服务端持续发布**、降负载而非原始 1080p，且在第 15 个采样点后因会话预算主动停止，未跑满 30 分钟。
