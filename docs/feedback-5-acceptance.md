...
## F5-05 多音轨 / per-source audio tracks — 逐轨探测/通道/前端混音与 Scene v6 持久化均已实现并实测
...
- **引擎侧仍只消费 legacy `audio_track`**：Composite 的逐轨混音（为每条选中输入轨建立独立 OBS 源实例并各自应用 gain/mute）尚未实现，因此 `audioInputs` 目前是“已持久化但尚未被引擎消费”的真实数据；逐轨同步偏移同样仍按来源处理。
