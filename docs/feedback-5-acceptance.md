...
- **服务端长稳（部分实测）**：真实五路来源降负载场景（960×540@5、软件渲染）连续运行 14 分钟、15 次每分钟采样：program 路径每次都 ready=True 且 tracks=['Opus','H264']，bytesReceived 由 182 KB 稳定增长到 57.9 MB（约 4 MB/分钟），webobsd 始终存活，source_recovery 与崩溃计数始终为 0（crashes=0 restarts=0），启动器报告 进程存活=是／引擎就绪=是／Program 发布=是／轨道=Opus,H264。该证据限于**服务端持续发布**、降负载而非原始 1080p，且在第 15 个采样点后因会话预算主动停止，未跑满 30 分钟。随后的补跑尝试未成功：启动器在 OBS 增量构建阶段后即退出（日志停在 NVENC 提示行，未出现 `[ERROR]`），推测与上一次残留会话争用同一原生锁/端口有关；已终止该次运行并恢复原始场景。随后在同一会话内再次重试一次，启动器同样在构建之后即退出（本次连 `[1/4]` 步骤都未写入日志，`dev-native.py` 进程消失而 soak 脚本仍在等待），判断为本会话反复启停后原生会话状态不再干净。本轮进一步定位：直接前台运行启动器，它在 MediaMTX 就绪探测阶段失败并打印 `[ERROR] mediamtx 未就绪`，而 `logs/mediamtx.log` **没有任何新记录**（尾部仍是首次 soak 的 02:00–02:15 旧内容），且此时 `pgrep` 已确认没有任何残留进程；说明该会话内 WSL 的监听套接字/网络状态已无法让 MediaMTX 重新绑定或让探测通过（本机亦无 `ss`，此前无法用端口检查提前发现）。结论：**代码与配置无回归**，仅当前 WSL 会话不再具备重复启动原生栈的能力；完整 30 分钟与浏览器侧长稳需在干净会话（必要时 `wsl --shutdown` 重启）或 GPU 桌面会话执行。长稳证据仍以这 14 分钟为准。
