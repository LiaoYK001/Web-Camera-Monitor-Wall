...
- **仍未测的是“客户端能否观测到”**：从 RTSP 客户端读取时视频/音频都被重新归零（相差约 −76 ms），因为接收端按各自 RTP 时间线表示；WebRTC 客户端本应通过 RTCP SR 恢复相对关系，但这一点尚未实测。要闭环需要“闪光 + 同步音脉冲”素材，在**浏览器内**用同一 `performance.now()` 时基同时检测画面闪光与音频脉冲——已列为下一步。
