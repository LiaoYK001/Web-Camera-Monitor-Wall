...
- **引擎侧（本轮修正，结论已改）**：
  ...
  - 逐轨同步偏移：`SceneAudioInput` 已增加 `syncOffsetMs`（±10000ms，解析时可选、保存时总写入、解析即校验范围；`validate_scene_document` 亦已加范围守卫），引擎把它作为 audio-mix 规格的第 4 字段传给网关，`transcode-on-demand.sh` 会在该轨 `volume` 之前插入 `adelay=<ms>|<ms>`（`f4191fe`、`5f6306a`）。仍缺：音频工作台的逐轨偏移控件，以及在双音轨合成上实测“延迟确实体现在节目音频里”。
