...
The full command creates `build/private-gates/windows.json` only after all required Chrome/Edge protocol, offline/update, resource-release and long-run checks pass.

完整命令只会在所有 Chrome/Edge 协议、离线/升级、资源释放与长稳必测项通过后生成 `build/private-gates/windows.json`。
