...
## 验收分类总览 / Acceptance summary

**已实现并自动化验证**：F5-01 填充优先级与几何统一、F5-02 画外来源状态区、F5-03 分能力探测 + OBS 编码器注册校验（修掉“外部 NVENC 探测冒充 OBS 编码器”导致 WHIP 发布失败的缺陷）、F5-04 启动器（`-Composite`/`--composite`、三级就绪、`--soak`、分阶段错误码）、F5-05 逐轨探测/逐轨 audio-only 通道/前端勾选与增益·静音·同步偏移/Scene v6 `audioInputs` 迁移与持久化/网关 `audio-mix` 逐轨混音、F5-06 播放状态机与探测缓存。套件：启动器 7/7、转码器 3/3 与 audio-mix 2/2、Camera Registry OK、C++ 单元测试全过、前端 22/22、`tsc --noEmit` 0 错误。

**已完成实机验证**：真实五路来源（降负载 960×540@5）→ OBS 合成 → H.264/Opus → MediaMTX → Program WHEP，无头浏览器连续解码 **41 帧/20 秒**（960×540，媒体时间持续前进）；Composite 逐轨混音：双音轨来源（440/880Hz）经网关 `amix` 后节目音频 **4 次抓取同时含两路频率**（无崩溃）；逐轨同步偏移 2 秒实测（前两秒只有一路）；**30 分钟服务端长稳：31 次采样全部 `program.ready=true` 且 `tracks=[Opus,H264]`，崩溃 0、来源重启 0，bytesReceived 0.37 MB → 104.9 MB，CPU ≈610–720%、内存 ≈4.6–4.8 GB，收尾后 MediaMTX 无残留 `mix-*` 路径、进程全部退出**。

**仅在低负载环境验证**：上述实机验证均在 **960×540@5** 的降负载场景；原始 1920×1080@30 在 Xvfb 软件渲染下是已记录的负面结果（渲染滞后 ~89%、输出 0 帧）。

**仍未验证**：≥30 分钟**浏览器侧**连续解码与 ≥90% 实际解码帧率；单路断开与恢复；CPU/GPU 归因（OBS 渲染/编码 vs 浏览器解码）；原始 1080p 的 WSLg/GPU 长稳；Docker/vGPU 验收（本轮按约定不验收）。

**明确的外部环境阻塞**：本沙箱仅有 `/dev/dxg` 且无 WSLg 图形会话（只能用 Xvfb 软件渲染），因此无法完成 GPU 渲染/编码归因与 1080p 长稳；环境内无 `gdb`、无 root（不能装调试器或改 `core_pattern`）、无 `ss`，环境级启动问题无法进一步定位。
...
- **服务端长稳（已完成 30 分钟，实测通过）**：31 次每分钟采样全部 `program.ready=true`、`tracks=[Opus,H264]`，`webobsd` 全程存活，崩溃与 `source_recovery` 计数始终为 0，bytesReceived 0.37 MB → 104.9 MB，CPU ≈610–720%、内存 ≈4.6–4.8 GB、GPU 3–16% 利用率；结束后无残留 `mix-*` 路径、无残留进程、场景已还原。启动器以 `--soak` 模式运行（不监听 stdin），此前“`[1/4]` 前静默退出”的问题已定位并修复（见 F5-04 启动器加固）。以下为更早的 14 分钟部分实测记录，保留作为过程证据：真实五路来源降负载场景（960×540@5、软件渲染）连续运行 14 分钟、15 次每分钟采样：program 路径每次都 ready=True 且 tracks=['Opus','H264']，bytesReceived 由 182 KB 稳定增长到 57.9 MB（约 4 MB/分钟），webobsd 始终存活，source_recovery 与崩溃计数始终为 0（crashes=0 restarts=0），启动器报告 进程存活=是／引擎就绪=是／Program 发布=是／轨道=Opus,H264。该证据限于**服务端持续发布**、降负载而非原始 1080p，且在第 15 个采样点后因会话预算主动停止，未跑满 30 分钟。随后的补跑尝试未成功：启动器在 OBS 增量构建阶段后即退出（日志停在 NVENC 提示行，未出现 `[ERROR]`），推测与上一次残留会话争用同一原生锁/端口有关；已终止该次运行并恢复原始场景。随后在同一会话内再次重试一次，启动器同样在构建之后即退出（本次连 `[1/4]` 步骤都未写入日志，`dev-native.py` 进程消失而 soak 脚本仍在等待），判断为本会话反复启停后原生会话状态不再干净。本轮进一步定位：直接前台运行启动器，它在 MediaMTX 就绪探测阶段失败并打印 `[ERROR] mediamtx 未就绪`，而 `logs/mediamtx.log` **没有任何新记录**（尾部仍是首次 soak 的 02:00–02:15 旧内容），且此时 `pgrep` 已确认没有任何残留进程；说明该会话内 WSL 的监听套接字/网络状态已无法让 MediaMTX 重新绑定或让探测通过（本机亦无 `ss`，此前无法用端口检查提前发现）。结论修正（后续实测）：单独直接运行 `mediamtx gateway/mediamtx.yml` **完全正常**——RTSP 8554、WebRTC 8889/8189、API 9997 三个监听器都能打开并优雅退出，说明本会话的网络/绑定能力并未损坏，之前启动器里的 `mediamtx 未就绪` 更可能是**上一次尝试仍在关闭过程中占用端口**（我连续启停时没有等待足够久）导致的瞬时冲突，而不是环境缺陷或代码回归。因此补跑 30 分钟长稳只需在**确认无残留进程**后单次启动即可。但按此方法在一次完全干净的状态下重试（第 106 轮）仍然失败：启动器在打印 NVENC 提示后、**在 `[1/4]` 之前就静默退出**（日志无异常行、`pgrep` 无 dev-native 进程，而该轮开始时已确认无任何残留进程；同一会话里 `mediamtx` 单独运行完全正常）。这说明反复启停之后本会话还有一处未定位的启动期问题，与产品代码无关。长稳证据仍以这 14 分钟为准；完整 30 分钟与浏览器侧长稳建议在**全新会话/机器**（或 GPU 桌面会话）中执行。
...
- **引擎侧（本轮修正，结论已改）**：
  ...
  - 仍需收尾（**纯清理，不影响行为**）：删除已被证否的“额外 OBS 抽取源”死代码——`attach_audio_input_instances`、`ensure_audio_only_path`、`AudioInputInstance`、`SourceEntry::audio_inputs` 字段、`is_ffmpeg_kind`、私有 `RuntimeState::audio_scene` 字段。`commit_prepared` 的空循环（`1bf5f24`）与复用路径里不可达的重建块（`27096be`）已删除。
  - 清理方法说明（供后续执行）：本文件**不能用脚本按大括号配对删除代码块**——`attach_audio_input_instances` 内部含有带大括号的字符串与初始化列表，脚本三次（标记法、括号配对、读后切片）都把块切错并导致编译失败（均已 `git checkout` 回滚，未进入历史）。可靠做法是：手工读取函数全文 → 用精确文本 `edit` 删除 → 编译 + 单元测试 → 仅在全绿时提交。
  - 逐轨同步偏移：`SceneAudioInput` 已增加 `syncOffsetMs`（±10000ms，解析时可选、保存时总写入、解析即校验范围；`validate_scene_document` 亦已加范围守卫），引擎把它作为 audio-mix 规格的第 4 字段传给网关，`transcode-on-demand.sh` 会在该轨 `volume` 之前插入 `adelay=<ms>|<ms>`（`f4191fe`、`5f6306a`）。前端：音频工作台每条选中音轨增加偏移输入框（`3d7a165`，从已保存的 `syncOffsetMs` 回填并在保存时写回）。**运行期实测（本轮）**：在 `mix-eeee…` 上用 `audio-mix 0:1.0:0:0,1:1.0:0:2000`（第二路延迟 2 秒）抓取 6 秒混音流，按 1 秒窗口做 Goertzel：前两窗口 440Hz≈1.6e7 而 880Hz≈9.3e3/1.2e3（尚未出现），第 3 窗口起两路均 ≈1.64e7（`EARLY880=9335 LATE440=16422874 LATE880=16434737 DELAY_EVIDENCE ok=1`）。即逐轨同步偏移确实按配置生效。
