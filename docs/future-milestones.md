...
>
> Current position / 当前位置：M8 NVR Core implementation is complete; M9 Timeline is in progress / M8 NVR 核心实现已完成，M9 时间线开发中
...
M6 established the production foundation, M7 completed Canvas Studio, and M8 completed the independent per-camera archive plane. M9 now makes those archives searchable, playable, and exportable; each later milestone still requires its predecessor's exit gate.

M6 建立生产底座，M7 完成画布工作台，M8 完成独立逐路归档平面。M9 现使这些归档可检索、可回放、可导出；后续每个里程碑仍以前一里程碑完成为前提。

```text
M6 Production      -> M7 Canvas Studio -> M8 NVR Core       -> M9 Timeline
M6 生产化（完成）  -> M7 画布工作台（完成） -> M8 NVR 核心（完成） -> M9 时间线（当前）
