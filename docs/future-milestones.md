...
>
> Current position / 当前位置：v1 is closed at `v1.2.1`; `dev` has completed `v2-M1` and is qualifying `v2-M2`. The production pipeline passes exact Qt 6.11.2/GStreamer 1.28.6 decoding for H.264/H.265 RTSP, Server Push MJPEG, HLS and WHEP with Docker control-network isolation, and 16 True Direct viewers coexist with one unchanged NVR upstream. Desktop Monitor/Studio, local operations, packaging and private hardware-gate tooling exist; signed Windows/Fedora packages and private 16+1 thirty-minute hardware evidence remain open / v1 已在 `v1.2.1` 收口；`dev` 已完成 `v2-M1`，正验收 `v2-M2`。生产管线已在 Docker 控制网络隔离下以固定 Qt 6.11.2/GStreamer 1.28.6 通过 H.264/H.265 RTSP、Server Push MJPEG、HLS 与 WHEP 解码，且 16 个真直连观看端可与一路不变的 NVR 上游共存；桌面 Monitor/Studio、本地操作、打包及私有硬件门禁工具已具备，Windows/Fedora 签名包和私有 16+1 三十分钟硬件证据仍待完成
