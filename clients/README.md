...
The private reference manifest contains exactly nine unauthenticated lab substreams at `640x360@15` and stays outside Git. A short-lived acceptance-driver APK is signed with the same release certificate, launches only the signature-protected probe Activity, and is never uploaded. `run-android-reference-gate.py` checks 30 minutes of MediaCodec decode, `<2%` drops, no black samples/restarts, no severe thermal state, unchanged Docker media sessions, and a second nine-stream background release within five seconds. Private evidence is atomically written outside the worktree with mode `0600`. Rotation, lock-screen, Wi-Fi/VPN handoff, microphone denial, reconnect, Grant expiry and revocation remain additional device-matrix evidence; source or simulator checks do not substitute for them.

私有参考清单严格包含九路 `640x360@15` 无认证实验室子码流并保存在 Git 之外。短期验收驱动 APK 使用同一发布证书签名，只能启动受 `signature` 权限保护的 Probe Activity，且绝不上传。`run-android-reference-gate.py` 检查 MediaCodec 持续解码 30 分钟、掉帧 `<2%`、无黑帧样本/管线重启、不进入严重温控、Docker 媒体会话不变，并在第二轮九路测试中验证退后台五秒内释放。私有证据以 `0600` 原子写到工作树之外。旋转、锁屏、Wi-Fi/VPN 切换、麦克风拒绝、重连、Grant 过期与撤销仍是额外实机矩阵证据；源码或模拟器检查不能替代它们。
