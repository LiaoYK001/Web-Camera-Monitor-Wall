...
The private reference manifest contains exactly nine unauthenticated lab substreams at `640x360@15` and stays outside Git. A short-lived acceptance-driver APK is signed with the same release certificate, launches only the signature-protected probe Activity, and is never uploaded. `run-android-reference-gate.py` checks 30 minutes of MediaCodec decode, `<2%` drops, no black samples/restarts, no severe thermal state and unchanged Docker media sessions. The same run performs four rotations, forces a five-second Wi-Fi outage and requires all nine production pipelines to reconnect within ten seconds, verifies a real user-managed `vpn → wifi` transition through silent runner-private helpers, verifies HOME release within five seconds and foreground recovery within ten seconds, verifies lock-screen release, checks Wake Lock cleanup, and proves microphone denial then grant. It accepts only a clean, USB-controlled reference device and removes both temporary APKs, the remote manifest, permissions and app-private state afterward. Private evidence is atomically written outside the worktree with mode `0600`. Client-side Grant expiry/revocation remains private device-matrix evidence; source or simulator checks do not substitute for it.

私有参考清单严格包含九路 `640x360@15` 无认证实验室子码流并保存在 Git 之外。短期验收驱动 APK 使用同一发布证书签名，只能启动受 `signature` 权限保护的 Probe Activity，且绝不上传。`run-android-reference-gate.py` 检查 MediaCodec 持续解码 30 分钟、掉帧 `<2%`、无黑帧样本/管线重启、不进入严重温控及 Docker 媒体会话不变；同一轮还执行四次旋转、强制断开 Wi-Fi 五秒并要求九条生产管线十秒内全部重连、通过静默且仅存在于 Runner 的 helper 验证真实用户自管 `VPN → Wi-Fi` 切换、验证 HOME 五秒内释放与前台十秒内恢复、锁屏释放、Wake Lock 清理及麦克风拒绝后授权。脚本只接受干净且由 USB 控制的专用参考设备，结束后移除两个临时 APK、远端清单、权限和应用私有状态。私有证据以 `0600` 原子写到工作树之外。客户端侧 Grant 过期/撤销仍需私有实机矩阵证据；源码或模拟器检查不能替代它。
