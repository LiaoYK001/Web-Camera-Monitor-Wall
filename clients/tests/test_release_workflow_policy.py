...
class NativeReleaseWorkflowPolicyTests(unittest.TestCase):
    ...
    def test_v3_preview_is_explicit_and_never_promotes_latest(self) -> None:
        ...
        self.assertIn('if [ "$version" = v3.0.1 ]; then', release_script)
        self.assertIn('default_prerelease_version="3.0.1-pre.1"', release_script)
        self.assertIn('default_prerelease_version="3.0.0-pre.1"', release_script)
        self.assertIn('build_version="${WEBOBS_PRERELEASE_BUILD_VERSION:-$default_prerelease_version}"', release_script)
