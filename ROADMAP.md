...
**🚧 v2-M1 release qualification + v2-M2 implementation / v2-M1 发布验收与 v2-M2 实施中。**

The `dev` branch contains the API v2 enrollment/Grant/topology contract and a functional Qt/QML desktop runtime. The production client pipeline now deterministically decodes H.264 and H.265 RTSP, Server Push MJPEG and HLS while Docker is control-network-only; continuous decoded-buffer evidence, crash-safe stream-copy MKV, MP4 remux and credential-free logs pass. Explicit Gateway/Hybrid fallback leases now activate an authenticated same-origin v2 WHEP route and clean up on close, release or revocation; forged, stolen and stale requests are rejected. Desktop source also includes 1/4/9/16 grids, independent main-stream focus, reconnect/suspend recovery, decoded-dimension diagnostics, local Studio Preview/Program, two-level nested scenes, transforms, basic filters, PTZ, listen, bounded talk, camera/local screenshots and manual recording. Hardware-decoder failure now rebuilds a software pipeline and restores process-wide decoder ranks safely after concurrent failures. Packaging verifies the actual locked source archives, isolates a self-contained GStreamer runtime, includes notices, and fails closed unless the packaged protocol/hardware/software-fallback element set is complete. A deterministic coexistence gate keeps one NVR upstream active while 16 concurrent True Direct viewers decode locally and verifies that server upstream/helper counts do not increase. The public native fixture also runs four production pipelines concurrently in one client process. The private 16+1 gate now uses one process and samples zero server media, decoded dimensions/FPS, hardware/drop status, sparse black-frame evidence, pipeline restarts and bounded RSS growth throughout 30 minutes. Exact locked-runtime decoded WHEP, signed Windows/Fedora artifacts and execution of that private hardware gate remain open. Therefore neither v2-M1 nor v2-M2 is marked complete, and no milestone/release tag is authorized.

`dev` 已包含 API v2 配对、Grant 与拓扑契约，以及可运行的 Qt/QML 桌面端。生产客户端管线已在 Docker 仅接控制网络时确定性直解 H.264/H.265 RTSP、Server Push MJPEG 与 HLS，并通过连续解码 Buffer、抗崩溃 stream-copy MKV、MP4 remux 和日志脱敏验证。显式 Gateway/Hybrid 后备租约现可激活受认证的同源 v2 WHEP 路由，并在关闭、释放或撤销时清理；伪造、盗用及陈旧请求均会拒绝。桌面源码还包含 1/4/9/16 宫格、独立主码流聚焦、重连/挂起恢复、实际解码分辨率诊断、本地 Studio Preview/Program、两级嵌套场景、变换与基础滤镜、PTZ、监听、有界对讲、摄像机/本地截图及手动录像。硬解码器失败现在会重建软件管线，并在多路并发失败后安全恢复进程级解码器优先级。打包会核验真实固定源码包、隔离自包含 GStreamer 运行时、附带声明，并在协议、硬解和软件回落插件不完整时拒绝产物；确定性共存门禁会保持一路 NVR 上游并让 16 个真直连观看端并发本地解码，确认服务端上游与 helper 数量没有增加；公开本地夹具还会在单个客户端进程内并发运行四条生产管线。私有 16+1 门禁现使用单进程，并在完整 30 分钟内持续采样服务端零媒体增量、实际分辨率/FPS、硬解/掉帧、稀疏黑帧、管线重启及有界 RSS 增长。固定运行时已解码 WHEP、Windows/Fedora 签名产物及这套私有硬件门禁的实际执行仍待完成，因此 v2-M1、v2-M2 均不标记完成，也不允许创建里程碑或发布 Tag。
