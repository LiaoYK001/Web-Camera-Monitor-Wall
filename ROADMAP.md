...
The `dev` branch contains the API v2 enrollment/Grant/topology contract and a functional Qt/QML desktop runtime. A Fedora exact-runtime build with Qt 6.11.2, GStreamer 1.28.6, libsodium 1.0.22 and the locked Rust WHEP commit decoded non-black H.264/H.265 RTSP, Server Push MJPEG, HLS and WHEP video. The isolated control and coexistence gates prove that 1–16 True Direct viewers add no Docker media helper or NVR upstream session, while forged, stale, replayed and revoked grants fail closed without leaking endpoints. This completes v2-M1. Topology responses are now strictly bound to the requested Camera/Profile and media owner, online revocation is checked on a five-second cadence, and supported ONVIF cameras provision and remove a dedicated User/Operator account with an explicit weak-revocation fallback if cleanup fails. Desktop source includes 1/4/9/16 grids, independent main-stream focus, reconnect/suspend recovery, local Studio Preview/Program, two-level nested scenes, transforms, basic filters, PTZ, listen, bounded talk, screenshots and manual stream-copy recording. Android source covers API 29+ arm64-v8a, adaptive Monitor, MediaCodec/thermal gating, foreground capture, fullscreen Wake Lock, PTZ/listen/talk, network re-evaluation, Android Keystore, private storage and SAF export. The private device gate now automates 30-minute nine-stream playback, rotation, forced Wi-Fi recovery, runner-private user-managed VPN handoff, HOME/lock release and foreground recovery, Android-Keystore enrollment with strict ten-second online revoke and bounded short-Grant expiry, Wake Lock and microphone denial/grant while cleaning device state. v2-M2 still needs signed Windows/Fedora packages and private 16+1 hardware evidence; v2-M3 still needs this complete signed arm64 hardware run. No v2.0 release tag is authorized until those gates actually run.

`dev` 已包含 API v2 配对、Grant、拓扑契约及可运行的 Qt/QML 桌面端。Fedora 精确运行时构建使用 Qt 6.11.2、GStreamer 1.28.6、libsodium 1.0.22 与固定 Rust WHEP 提交，已解码 H.264/H.265 RTSP、Server Push MJPEG、HLS 与 WHEP 非黑画面。隔离控制与 NVR 共存门禁证明 1～16 个真直连观看端不会增加 Docker 媒体 helper 或 NVR 上游会话；伪造、陈旧、重放及撤销 Grant 均安全拒绝且不泄漏端点，因此 v2-M1 已完成。TopologyPlan 响应现严格绑定请求的 Camera/Profile 与媒体所有者，在线撤销按五秒周期复验；支持用户管理的 ONVIF 摄像机还会真实创建和删除专用 User/Operator 账号，清理失败则明确回落为弱撤销。桌面源码包含 1/4/9/16 宫格、独立主码流聚焦、重连/挂起恢复、本地 Studio Preview/Program、两级嵌套场景、变换、基础滤镜、PTZ、监听、有界对讲、截图与 stream-copy 手动录像。Android 源码支持 API 29+ arm64-v8a、自适应 Monitor、MediaCodec/温控门禁、前台录像、全屏常亮、PTZ/监听/按键对讲、网络切换复判、Android Keystore、私有存储及 SAF 导出；私有实机门禁现已自动覆盖九路 30 分钟、旋转、强制 Wi-Fi 恢复、Runner 私有用户自管 VPN 切换、HOME/锁屏释放与前台恢复、Android Keystore 配对、严格十秒在线撤销及有界短 Grant 到期、Wake Lock 与麦克风拒绝/授权，并清理设备状态。v2-M2 仍缺 Windows/Fedora 签名包及 16+1 实机证据；v2-M3 仍缺上述完整签名 arm64 实机执行；这些门禁真实执行前不允许发布 v2.0 Tag。

Desktop focus/minimize now keeps monitoring active, while a real application suspension rebuilds streams; Android retains foreground-only media. The deterministic True Direct gate now drives the production background/foreground trigger, releases media in 3 ms in the current fixture, resumes it, and still proves zero incremental server media. The private desktop gate additionally requires a real system sleep, hardware-decoder failure recovery and a fresh second-process offline startup from a DPAPI/Secret Service-sealed Grant. Signed Windows/Linux install and rollback qualification plus immutable Release-asset publication are wired into the release workflow. These source gates pass; private hardware and signing execution remains outstanding / 桌面失焦或最小化现会继续监看，真正的应用挂起才重建流；Android 仍仅在前台运行媒体。确定性真直连门禁现已驱动生产后台/前台触发路径，在当前夹具中 3 ms 释放媒体、完成恢复，且仍证明服务端媒体增量为零。桌面私有门禁另行强制真实系统休眠、硬解码器故障回落，以及从 DPAPI/Secret Service 密封 Grant 启动全新第二进程的离线播放。发布工作流也已接入 Windows/Linux 签名安装与回滚验收及不可变 Release 资产发布。上述源码门禁已通过，私有硬件与签名实跑仍待完成。
