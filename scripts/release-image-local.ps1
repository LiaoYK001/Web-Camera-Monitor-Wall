...
if ($Version -eq 'dev') {
    $tags += @('--tag', "${Image}:dev")
    $buildVersion = "2.3.0-dev.${shortRevision}"
    $buildMilestone = 'v2-M7-dev'
}
else {
    $tags += @('--tag', "${Image}:latest")
    $buildVersion = $Version.Substring(1)
    if ($buildVersion -match '^[0-9]+\.[0-9]+$') { $buildVersion = "${buildVersion}.0" }
    $buildMilestone = if ($Version -match '^v2\.3(?:\.|$)') { 'v2-M7' }
        elseif ($Version -match '^v2\.2(?:\.|$)') { 'v2-M6' }
