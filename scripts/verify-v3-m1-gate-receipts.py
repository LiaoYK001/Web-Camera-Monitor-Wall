#!/usr/bin/env python3
"""Fail closed unless revision-bound v3-M1 private gates are present."""
...
CONTRACT = "webobs-v3-m1-gate-receipt-v1"
SPECS = {
...
def fail(message: str) -> None:
    raise SystemExit(f"v3-M1 release gate rejected: {message}")
...
def main() -> int:
    ...
    for name, expected in SPECS.items():
        ...
        if path.is_symlink() or not path.is_file() or path.stat().st_size > 64 * 1024:
            fail(f"missing or invalid {name}")
        ...
        if value.get("revision") != revision or not isinstance(value.get("checks"), list) or set(value["checks"]) != expected or len(value["checks"]) != len(expected):
            fail(f"{name} revision or checks mismatch")
        ...
        if completed.tzinfo is None or completed > now + timedelta(minutes=5) or now - completed > timedelta(hours=48):
            fail(f"{name} is outside the 48 hour validity window")
    print(f"v3-M1 release receipts passed for revision {revision[:12]}.")
    return 0
