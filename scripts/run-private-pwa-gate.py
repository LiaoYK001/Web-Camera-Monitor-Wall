#!/usr/bin/env python3
"""Run a runner-local v2 PWA media gate without publishing private evidence."""
...
def main() -> int:
    ...
    if not raw_command:
        fail("runner variable WEBOBS_PRIVATE_PWA_GATE_COMMAND is not configured")
    ...
    if is_within(command, workspace):
        fail("command must be runner-local and outside GITHUB_WORKSPACE")
    if os.name != "nt" and not command.stat().st_mode & stat.S_IXUSR:
        fail("runner-local command is not executable")
        ...
        with log_path.open("wb") as log:
            ...
            while process.poll() is None:
                ...
                if log_path.stat().st_size > MAX_LOG_BYTES:
                    process.kill()
                    process.wait()
                    fail("runner-local command exceeded the private log limit")
                if time.monotonic() >= deadline:
                    process.kill()
                    process.wait()
                    fail("runner-local command timed out")
            if process.returncode != 0:
                fail("runner-local command failed; rerun it locally to inspect private output")
