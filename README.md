...
开发路线和门禁见 [ROADMAP.md](ROADMAP.md)。当前 API v1 的 `direct` 仍是媒体经过 Docker/MediaMTX 的“网关直通”；获批的 HTTPS WHEP/HLS/MJPEG 可 Camera→Browser，普通 RTSP 明确回退 Camera→Docker→Browser。v2-M4 提供加密增量同步，v2-M5 提供统计、1–16 M/S 布局与低功耗；`dev` 正在实现 v2-M6 的运维工作区、来源目录、问题中心、独立预览和逐源音频工作台。Qt/GStreamer/Android 继续冻结，不新增 EXE/APK 发布门禁。详见 [v2.2 运维工作区](docs/operations-workspace-v2.2.md)、[MonitorView 与分析交接](docs/monitor-view-and-analytics.md)、[Local-first PWA](docs/local-first-pwa.md)、[本机 Windows/WSL2 门禁](docs/local-platform-gates.md)、[API v2](docs/api-v2.md) 与 [真直连边界](docs/true-direct-v2.md)。
