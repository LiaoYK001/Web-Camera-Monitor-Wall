...
开发路线和门禁见 [ROADMAP.md](ROADMAP.md)。v1-M10/M11 的 Digest/WS-Security/TLS、事件、Webhook/MQTT 及 Server Push MJPEG 契约已通过确定性测试；另有一个脱敏 Canon WV-HTTP 外部端点通过 MIME、媒体协商及五帧解码门禁。多厂商型号/固件矩阵改为持续兼容项目，不构成品牌级或 ONVIF 合规声明，详见 [摄像机兼容性资格](docs/camera-compatibility-qualification.md)。当前 API v1 的 `direct` 是媒体仍经过 Docker/MediaMTX 的“网关直通”。`dev` 已完成 v2-M1，v2-M2/M3 已转向 Local-first PWA 与 Browser Media Runtime：浏览器缓存完整应用壳及加密、限期配置；获批的 HTTPS WHEP/HLS/MJPEG 可 Camera→Browser，普通 RTSP 明确回退 Camera→Docker→Browser。Qt/GStreamer/Android 代码冻结且不再作为 v2.0 发布门禁；v2.0 只发布内含正式 PWA 的 GHCR 镜像。详见 [Local-first PWA](docs/local-first-pwa.md)、[API v2](docs/api-v2.md) 与 [真直连边界](docs/true-direct-v2.md)。
