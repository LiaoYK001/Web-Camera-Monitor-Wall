...
开发路线和门禁见 [ROADMAP.md](ROADMAP.md)。v1-M10/M11 的 Digest/WS-Security/TLS、事件、Webhook/MQTT 及 Server Push MJPEG 契约已通过确定性测试；另有一个脱敏 Canon WV-HTTP 外部端点通过 MIME、媒体协商及五帧解码门禁。多厂商型号/固件矩阵改为持续兼容项目，不构成品牌级或 ONVIF 合规声明，详见 [摄像机兼容性资格](docs/camera-compatibility-qualification.md)。当前 `direct` 是媒体仍经过 Docker/MediaMTX 的“网关直通”。`dev` 已完成 v2-M1 并进入 v2-M2/v2-M3 发布验收：配对、加密 Grant、严格拓扑规划、ONVIF 专用账号生命周期、管理界面、本地桌面/Android 客户端及隔离多协议架构夹具均已具备；1～16 个真直连观看端与单路 NVR 共存通过确定性零增量门禁，固定 Qt 6.11.2/GStreamer 1.28.6 运行时也已解码 RTSP H.264/H.265、Server Push MJPEG、HLS 与 WHEP。尚缺 Windows 11、当前/上一 Fedora 的签名产物及 16+1 三十分钟真实硬件证据，以及签名 Android arm64 包和九路三十分钟实机证据，因此尚未发布 v2.0。详见 [API v2](docs/api-v2.md)、[真直连边界](docs/true-direct-v2.md) 与 [本地客户端](clients/README.md)。
