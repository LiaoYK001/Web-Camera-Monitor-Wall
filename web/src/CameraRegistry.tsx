...
import { createCamera, deleteCamera, detectCamera, discoverOnvif, fetchCameras } from './api';
...
export default function CameraRegistry({ onBack }: { onBack: () => void }) {
  ...
  const add = async () => {
    ...
    try {
      await createCamera({ name: name.trim(), address: detection.address, adapter: detection.adapter,
        ...
        capabilities: { probe: detection.probe, contentType: detection.contentType ?? '' } });
  ...
  return <main className="registry-page">
    ...
    <section className="registry-add">
      ...
      <label><span>地址</span><input value={address} placeholder="camera.example.invalid 或 rtsp://camera.example.invalid/live" onChange={(event) => setAddress(event.target.value)} /></label>
      ...
      {detection && <div className="detection-result"><strong>{detection.adapter.toUpperCase()} · {detection.probe}</strong><span>{detection.profiles.length ? `${detection.profiles.length} 个码流 Profile` : '等待设备授权后读取 Profile'}</span><label><span>设备名称</span><input value={name} maxLength={128} onChange={(event) => setName(event.target.value)} /></label><label><span>凭据 Secret 引用（可选）</span><input value={credentialsRef} maxLength={256} placeholder="front-door" onChange={(event) => setCredentialsRef(event.target.value.replace(/[^a-zA-Z0-9._/-]/g, ''))} /></label><small>引用 /run/secrets/webobs-camera-credentials/&lt;名称&gt;.json；数据库不保存密码。</small><button className="primary-button" disabled={!name.trim()} type="button" onClick={() => void add()}>保存到 Registry</button></div>}
      {discovered.length > 0 && <div className="discovery-list">{discovered.map((device) => <button type="button" key={device.address} onClick={() => setAddress(device.address)}><strong>{device.host}</strong><span>{device.address}</span></button>)}</div>}
    </section>
    <section className="camera-list"><div className="section-title"><h2>Camera Registry</h2><span>{cameras.length} 台</span></div>
      {cameras.length === 0 ? <div className="registry-empty"><h3>尚未添加摄像机</h3><p>使用自动检测，或通过 ONVIF WS-Discovery 查找局域网设备。</p></div> : cameras.map((camera) => <article className="camera-card" key={camera.id}><div><span className="adapter-pill">{camera.adapter}</span><h3>{camera.name}</h3><p>{camera.address}</p></div><dl><div><dt>Profile</dt><dd>{camera.profiles.length}</dd></div><div><dt>硬解</dt><dd>{camera.hardwareDecode}</dd></div><div><dt>健康</dt><dd>{camera.health}</dd></div></dl><div className="profile-list">{camera.profiles.map((profile) => <span key={profile.id}>{profile.role} · {profile.videoCodec || 'unknown'} {profile.width ? `${profile.width}×${profile.height}` : ''}</span>)}</div><button className="danger-button" type="button" onClick={() => { if (window.confirm(`删除 ${camera.name}？`)) void deleteCamera(camera.id).then(reload); }}>删除</button></article>)}
    </section>
